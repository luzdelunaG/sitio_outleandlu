...
class BlogControl extends Lean{
		function index(){}
		...
		function admin_index(){
			 $this->Send("title", "Vista de Blog");
			 $this->Send("subtitle", "/Blog");
		}

		function admin_editar($id,$nombre){
			 $this->Send("title", "Editar página Blog");
			 $this->Send("subtitle", "/Blog");
			 $this->Send("id",$id);
		}
		...
		function api_getdatos($id = false){
			 if(!$id){
					 ...
					 return $this->Blog->onlyByIdBlog($id);
			 }
		}

		function api_guardar($id = false){
			 if(count($this->Post)){
					 $ruta = ROOT."fileimages".DS."Blog".DS;
					 if(!$id){
							 $b=$this->Blog->query('select MAX(id_blog) AS id FROM blog');
							 $b=$b[0]["id"]+1;
							 ...
							 $r = $this->Blog->modify($this->Post, ["id_blog"=>$id]);
					 }
		 ...
		 function api_ordena(){
			 if($this->Post){
					 ...
					 foreach ($new as $cv => &$value) {
							 $cv = $cv+1;
							 $result = $this->Blog->modify(array("orden"=>$cv),array("id_blog"=>$value));
					 }
			 }
		}
