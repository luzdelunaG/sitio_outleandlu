...
(function( $ ){
	$.fn.opciones = function(options) {
		...
		_Elemento.before("<div class='dataTables_length'>Mostrar <select> <option value='15' selected='selected'>15</option><option value='30'>30</option><option value='50'>50</option><option value='100'>100</option></select>entradas</div><div class='dataTables_filter'>Buscar: <input type='text' class='txtSearch' placeholder='Buscar datos' /></div>");
		...
		_Th.click(function(){
			...
			_th.attr("class", "desc");
			_tag_i.attr("class", "icon-chevron-up");
				
			if(_class == "desc"){
				_datos.reverse();
				_th.attr("class", "asc");
				_tag_i.attr("class", "icon-chevron-down");
			}
