    ...
    JForm.prototype.onChangeFile = function(element, evt) {
        ...
        $.each(files, function(index, file){
            ...
            reader.onload = function(e) {
                var img = $("<img class='imgCharge' />");
                if (e.target.result.search(/jpeg/)!=-1 || e.target.result.search(/png/)!=-1 || e.target.result.search(/gif/)!=-1 ){

                    //img.attr('src', e.target.result);
                }else{
                    img.attr('src', "templates/boot/images/pdf.png");
                }
                divPrev.html(img);
            }
